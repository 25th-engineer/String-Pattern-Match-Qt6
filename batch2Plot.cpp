...
#include "ui_batch2Plot.h"

batch2plot::batch2plot(std::vector< std::pair<QString,int> > timeUsedVector,
                       QString testBench, QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::batch2plot)
{
    ui->setupUi(this);
    this->setWindowTitle("Plot for Batch Mode 2");
    ...
    chart->setTitle(QString("Time Consuming Comparison of BF, RK, KMP and BM over "
                            + testBench));
...
batch2plot::~batch2plot()
{
    delete ui;
}

int batch2plot::getLength(int num)
{
    int lgth =0;
    while(num)
    {
...
void batch2plot::on_actionSave_as_PNG_image_triggered()
{
...
void batch2plot::on_actionStartup_triggered()
{
    this->close();
...
void batch2plot::on_actionExit_triggered()
{
    this->close();
}
